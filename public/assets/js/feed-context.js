const FeedContext = {
                    ...
                    <div class="p-3">
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-4 gap-3">
                                <button onclick="return FeedContext.copyPostLink(${post.post_id}, '${post.post_uuid}')" class="flex items-center justify-center space-x-2 px-4 py-3 bg-blue-300 text-white rounded-xl hover:bg-blue-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                                    </svg>
                                </button>
                                <button onclick="return FeedContext.socialMediaShare('whatsapp', '${shortText}', ${post.post_id}, '${post.post_uuid}')" class="flex items-center justify-center space-x-2 px-4 py-3 bg-green-500 text-white rounded-xl hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors">
                                ...
                                <button onclick="return FeedContext.socialMediaShare('twitter', '${shortText}', ${post.post_id}, '${post.post_uuid}')" class="flex items-center justify-center space-x-2 px-4 py-3 bg-blue-400 text-white rounded-xl hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 transition-colors">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                                    </svg>
                                </button>
                                <button onclick="return FeedContext.socialMediaShare('facebook', '${shortText}', ${post.post_id}, '${post.post_uuid}')" class="flex items-center justify-center space-x-2 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 transition-colors">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    socialMediaShare(platform, text, post_id, post_uuid) {
        const postUrl = `${window.location.origin}/shared/posts/${post_uuid}/${post_id}`;
        let shareUrl = '';
        switch(platform) {
            case 'whatsapp':
    ...
    copyPostLink(post_id, post_uuid) {
        const postUrl = `${window.location.origin}/shared/posts/${post_uuid}/${post_id}`;
